"""Parser for ``webgpu.idl`` -- the W3C WebGPU Web IDL.

Two specs drive this project, and they answer different questions:

* ``webgpu.json`` (from the wgpu-native submodule) describes the **C API**:
  which functions exist, their C names, argument types and struct layouts. It
  is what the FFI layer is generated from.
* ``webgpu.idl`` (from the W3C spec) describes the **public API**: the class,
  method and attribute names, the *string* values of enums, and the struct
  field names and defaults that user code actually writes.

Deriving the public API from the C spec alone does not work: the C spec spells
enum entries as ``RGBA8_unorm`` / ``2D_array`` where the web API -- and every
existing wgpu-py user -- says ``"rgba8unorm"`` / ``"2d-array"``. The IDL is the
authority for that, so it stays the source of truth for the public surface.

Adapted from the historical ``codegen/idlparser.py``, decoupled from the rest
of that package.
"""
...
def get_idl_parser(*, allow_cache=True):
    """Get the global :class:`IdlParser`, parsed once per process."""
    global _parser
    if _parser and allow_cache:
        return _parser
    idl = IdlParser(paths.WEBGPU_IDL.read_text())
